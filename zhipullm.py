from typing import Any, Iterator, Dict,List, Optional, Mapping
...
class ZhipuLLM(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        time_in_second=time.time()-start_time

        message=AIMessage(
            content=response.choices[0].message.content,
            additional_kwards={},
            response_metadata={
                "耗时": round(time_in_second,3),
            },
            usage_metadata={
                "输入Token":response.usage.prompt.tokens,
                "输出Token":response.usage.completion.tokens,
                "总共Token":response.usage.total.tokens,
            })
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        for res in response:
            ...
            if run_manager:
                run_manager.on_llm_new_token(res.choices[0].delta.content)
            yield chunk
        time_in_second=time.time()-start_time
        chunk=ChatGenerationChunk(
            message=AIMessageChunk(content="",response_metadata={"耗时":round(time_in_second,3)},usage_metadata=usage_metadata)
        )
        if run_manager:
...
if __name__ == "__main__":
    # Test
    model = ZhipuaiLLM(model_name="glm-4-plus",api_key=os.environ["ZHIPUAI_API_KEY"])
    # invoke
